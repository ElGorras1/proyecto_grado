...
const vistaActual = ref<'buscar' | 'transaccion' | 'nuevo_producto'>('buscar')
...
const transaccion = ref({ tipo: 'salida', cantidad: 1, nodo_grafo: '', observaciones: '' })
const creandoTransaccion = ref(false)
const nodosDisponibles = ref<string[]>([])
...
async function cargarNodos() {
  try {
    const res = await api.get('/kardex/nodos')
    nodosDisponibles.value = res.data
  } catch (err) {
    console.error("Error al cargar nodos", err)
  }
}

function iniciarTransaccion(art: Articulo) {
  articuloSeleccionado.value = art
  vistaActual.value = 'transaccion'
  transaccion.value = { tipo: 'salida', cantidad: 1, nodo_grafo: '', observaciones: '' }
  errorMsg.value = ''
  mensajeExito.value = ''
}
...
async function guardarNuevoProducto() {
  ...
  try {
    ...
    // 2. Registrar saldo inicial como inventario actual
...
async function registrarTransaccion() {
  ...
  if (!transaccion.value.nodo_grafo) return (errorMsg.value = "Selecciona un destino/origen.")
  if (transaccion.value.cantidad <= 0) return (errorMsg.value = "La cantidad debe ser mayor a 0.")
  ...
  try {
    await api.post('/kardex/movimiento-actual', {
      ...
      nodo_grafo: transaccion.value.nodo_grafo,
      observaciones: transaccion.value.observaciones
    })
    mensajeExito.value = "Transacción registrada correctamente en el inventario actual."
    setTimeout(volver, 2000)
  } catch (err) {
...
<template>
  <div class="inventario-actual-view">
    <div class="header-nav">
      ...
      <button v-else class="btn-back" @click="volver"><i class="pi pi-arrow-left"></i> Volver</button>
      
      <h1>Inventario Actual (Operaciones en Vivo)</h1>
      <div style="flex-grow:1"></div>
      
      <button v-if="vistaActual === 'buscar'" class="btn-primary" @click="irANuevoProducto">
        + Nuevo Producto (Presente)
      </button>
    </div>
    ...
    <div v-if="vistaActual === 'buscar'" class="pantalla-buscar">
      <p class="subtitle">Registra movimientos modernos sobre los activos existentes, o crea nuevos activos.</p>
      <div class="search-box">
        <i class="pi pi-search search-icon"></i>
        <input type="text" v-model="buscarTexto" placeholder="Buscar producto para registrar movimiento..." class="search-input"/>
      </div>

      <div class="lista-articulos">
        <div v-for="art in articulosFiltrados" :key="art.id" class="articulo-card" @click="iniciarTransaccion(art)">
          <div class="art-header">
            <div class="art-codigo">{{ art.codigo }}</div>
          </div>
          <div class="art-nombre">{{ art.nombre }}</div>
          <button class="btn-outline btn-small mt-3 w-full">Nueva Transacción <i class="pi pi-arrow-right"></i></button>
        </div>
      </div>
    </div>
    ...
    <div v-if="vistaActual === 'transaccion' && articuloSeleccionado" class="form-container">
      <h3>Registrar Transacción en Vivo</h3>
      <div class="context-box">
        <strong>Activo:</strong> [{{ articuloSeleccionado.codigo }}] {{ articuloSeleccionado.nombre }}
      </div>
      
      <div class="form-group mt-3">
        <label>Tipo de Operación:</label>
        <select v-model="transaccion.tipo" class="w-full">
      ...
      <div class="form-group mt-3">
        ...
        <input type="number" v-model="transaccion.cantidad" min="1" class="w-full" />
      </div>
      
      <div class="form-group mt-3">
        <label>Destino / Origen (Nodo):</label>
        <select v-model="transaccion.nodo_grafo" class="w-full">
          <option disabled value="">Selecciona el destino/origen...</option>
          <option v-for="nodo in nodosDisponibles" :key="nodo" :value="nodo">{{ nodo }}</option>
        </select>
        <p class="hint">Estos destinos se alimentan de la historia de los Kárdex escaneados.</p>
      </div>
      
      <div class="form-group mt-3">
        <label>Observaciones Adicionales:</label>
        <input type="text" v-model="transaccion.observaciones" placeholder="Referencia, número de orden, persona..." class="w-full" />
      </div>
